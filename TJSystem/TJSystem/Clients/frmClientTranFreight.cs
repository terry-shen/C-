...
namespace TJSystem.Clients
{
	...
	/// Form1 的摘要说明。
	/// </summary>
	public class frmClientTranFreight : System.Windows.Forms.Form
	{
		...
		public frmClientTranFreight(string supplier)
		{
			//
			// Windows 窗体设计器支持所必需的
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmClientTranFreight));
			...
			this.Name = "frmClientTranFreight";
		...
		private bool checkFormValue(string sqltype)
		{
			...
			if(!TJSystem.Public.PublicStatic.IsNumDouble(priTextBox12.Text.Trim()))
			{
				TJSystem.Public.PublicStatic.ShowMessage(TJSystem.Public.MessageType.Error,"最小订货量必须是数字！");
				return false;
			}

			if(!TJSystem.Public.PublicStatic.IsNumDouble(priTextBox9.Text.Trim()))
			{
				TJSystem.Public.PublicStatic.ShowMessage(TJSystem.Public.MessageType.Error,"采购价格必须是数字！");
				return false;
			}
			if(!TJSystem.Public.PublicStatic.IsNumDouble(priTextBox5.Text.Trim()))
			{
				TJSystem.Public.PublicStatic.ShowMessage(TJSystem.Public.MessageType.Error,"销售价格必须是数字！");
				return false;
			}
			return true;
		}
