...
namespace TJSystem.Sales
{
	...
	/// Form1 的摘要说明。
	/// </summary>
	public class frmSSDetail : System.Windows.Forms.Form
	{
		...
		private string _Sale_SS_Code = null;
		...
		public frmSSDetail(string sale_SS_code)
		{
			//
			// Windows 窗体设计器支持所必需的
			//
			...
			_statusType = "sales";
			this._Sale_SS_Code = sale_SS_code;
			this.txt1.Text = _Sale_SS_Code;
			...
			sb.Append("select a.ss_code,a.seq_no,a.item_code,a.client_itemcode,a.item_name,a.item_name_cn,a.item_desc,a.item_desc_cn,a.item_qty,b.unit_name_cn as unit,");
			sb.Append("a.remark ");
			sb.Append("from sale_ss_detail a ");
			sb.Append("left join p_unit b ");
			sb.Append("on a.unit = b.unit_id ");
			sb.Append(" where ss_code = '"+ this.txt1.Text.ToString().Trim() +"'");
			sb.Append(" order by ss_code,seq_no");
			this.InitialGrid(sb.ToString());

			priToolBar1.setCurrentForm = this;
			_PK = "ss_code,seq_no";
            _table = "sale_ss_detail";
		...
		private void InitialFormData()
		{
			...
			sql = "select a.client_itemcode as client_itemcode,a.client_itemcode +' '+c.client_name as client_name from basic_clientItemCode a join basic_item b on a.item_code = b.item_code join basic_client c on c.client_id = a.client_id order by a.client_itemcode";
...
//					MessageBox.Show(sb.ToString(),"Error",MessageBoxButtons.OK,MessageBoxIcon.Error);
		...
		private bool checkFormValue(string sqltype)
		{
            ...
            {
                TJSystem.Public.PublicStatic.ShowMessage(TJSystem.Public.MessageType.Error, "请填写退货数量！");
                return false;
            }
            if (!TJSystem.Public.PublicStatic.IsNumDouble(this.txt7.Text.Trim()))
            {
                TJSystem.Public.PublicStatic.ShowMessage(TJSystem.Public.MessageType.Error, "退货数量必须为数字！");
                return false;
            }	
			return true;
		}
		...
		private void AfterButtonClick(string ButtonType)
		{
			...
			{
				string sql = "select max(seq_no) as seq_no from sale_ss_detail where ss_code = '"+ _Sale_SS_Code +"'";
				DataTable dt = TJSystem.Public.PublicStatic.db.GetDataTable(sql);
...
//		}
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmSSDetail));
			...
			this.dataGrid1.CaptionText = "销售退货明细";
			...
			this.dataGridTableStyle1.MappingName = "sale_ss_detail";
			// 
			// ce_code
			// 
			this.ce_code.Format = "";
			this.ce_code.FormatInfo = null;
			this.ce_code.HeaderText = "销售退货单号";
			this.ce_code.MappingName = "ss_code";
